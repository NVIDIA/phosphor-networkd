                                     ...
                                     size_t* resp_msg_len)
{
    ...
    ssize_t min_len = sizeof(msgTag) + sizeof(eid) +
                      sizeof(MCTP_MSG_TYPE_NCSI) + sizeof(struct ncsi_pkt_hdr);
                                   ...
                                   size_t* resp_msg_len)
{
    ...
    ncsi_requester_rc_t rc = ncsi_send(eid, mctp_fd, ncsi_req_msg, req_msg_len);
    if (rc != NCSI_REQUESTER_SUCCESS)
    {
        return rc;
    }
    ...
    while (1)
    {
        rc = ncsi_recv(eid, mctp_fd, hdr->id, ncsi_resp_msg, resp_msg_len);
